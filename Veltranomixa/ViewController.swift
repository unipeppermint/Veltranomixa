...
final class ViewController: UIViewController {
    enum Page { case island, levels, game, collection, settings }
    ...
    private var latestMessage = "转动转盘，让好运靠岸"
    private var selectedUpgrade: Upgrade?
    override func viewDidLoad() {
        super.viewDidLoad(); overrideUserInterfaceStyle = .light
        let directory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appendingPathComponent("LuckyIsland", isDirectory: true)
    ...
    private func render() {
        ...
        scroll.snp.makeConstraints { $0.top.leading.trailing.equalTo(view.safeAreaLayoutGuide); if page == .game { $0.bottom.equalTo(view.safeAreaLayoutGuide) } else { $0.bottom.equalTo(nav.snp.top).offset(-8) } }
        ...
        switch page { case .island: island(); case .levels: levels(); case .game: game(); case .collection: collection(); case .settings: settings() }
    }
    ...
    private func island() {
        heading("LUCKY ISLAND  /  把幸运变成风景", "幸运小岛", "不急着远航，先让这座小岛热闹起来。")
    ...
    private func detail(_ id: Int) {
        ...
        a.addAction(UIAlertAction(title: "开始挑战", style: .default) { [weak self] _ in
            guard let self else { return }; if self.commit({ try GameEngine.start(id, in: &$0) }) { self.latestMessage = "转动转盘，让好运靠岸"; self.go(.game) }
    ...
    private func game() {
        ...
        #if DEBUG
        if ProcessInfo.processInfo.environment["ISLAND_UI_FIXED_WOOD"] == "1" { add(Theme.label("自动测试模式 · 固定木材结果", size: 12, color: .systemRed)) }
        #endif
        ...
        let craft = Theme.button("改造转盘 · 木材格 +1", symbol: "hammer.fill") { [weak self] in self?.craft() }; craft.isEnabled = r.pending == nil; add(craft)
    ...
    private func spin() {
        ...
        if state.run?.pending == nil {
            ...
            if ProcessInfo.processInfo.environment["ISLAND_UI_FIXED_WOOD"] == "1" {
                struct FixedWood: RandomSource { mutating func nextIndex() -> Int { 0 } }
                var rng = FixedWood()
    ...
    private func upgrades(_ r: RunState) {
        heading("发现新的可能", "选择一项升级", "\(latestMessage)\n升级仅在当前挑战内生效。")
    ...
    private func result(_ r: RunState) {
        let won = r.phase == .won
        heading(won ? "又一份幸运，成为小岛的风景" : "海风会带来下一次机会", won ? "\(Content.levels[r.level].name) · 完成" : "差一点就完成了", won ? (r.level < 12 ? "建设成果已自动加入小岛与收藏。" : (r.level < 14 ? "新转盘外观已加入收藏，可前往衣橱使用。" : "小岛庆典成就已解锁，谢谢你为小岛带来这么多风景。")) : "试试更早加固木材格，或用金币替换格子。")
        if r.level < 12 {
    ...
    private func craft() {
        guard let r = state.run, r.pending == nil else { return }
        let a = UIAlertController(title: "转盘工坊 · \(r.coins) 金币", message: "改造仅本局生效。格子替换不改变每格 12.5% 的概率。", preferredStyle: .alert)
        a.addAction(UIAlertAction(title: "4 金币 · 所有木材格 +1", style: .default) { [weak self] _ in self?.buyCraft() })
        if r.level > 0 { a.addAction(UIAlertAction(title: "6 金币 · 替换一个格子", style: .default) { [weak self] _ in self?.replacement() }) }
        a.addAction(UIAlertAction(title: "返回", style: .cancel)); present(a, animated: true)
    }
    private func buyCraft(index: Int? = nil, kind: Tile = .wood) {
        if commit({ try GameEngine.craft(&$0, replacing: index, with: kind) }) { latestMessage = "改造完成，新的策略已就绪"; Feedback.shared.play(state.settings); render() }
    }
    private func replacement() {
        guard let r = state.run else { return }
        let a = UIAlertController(title: "选择要替换的格子", message: "从顶部开始，按顺时针编号。", preferredStyle: .alert)
        for (i,t) in r.wheel.enumerated() { a.addAction(UIAlertAction(title: "\(i+1) · \(t.kind.title)", style: .default) { [weak self] _ in self?.replacementKind(i) }) }
        a.addAction(UIAlertAction(title: "取消", style: .cancel)); present(a, animated: true)
    }
    private func replacementKind(_ index: Int) {
        let a = UIAlertController(title: "新格子 · 花费 6 金币", message: "新格子基础产出 2，可获得对应工具加成。", preferredStyle: .alert)
        for kind in (state.run?.level ?? 0) >= 5 ? [Tile.wood,.coin,.shell] : [.wood,.coin] { a.addAction(UIAlertAction(title: "\(kind.title) +2", style: .default) { [weak self] _ in self?.buyCraft(index: index, kind: kind) }) }
        a.addAction(UIAlertAction(title: "取消", style: .cancel)); present(a, animated: true)
    }
    ...
    private func tutorial() {
        let a = UIAlertController(title:"欢迎来到幸运小岛",message:"① 选择建设目标，转动八格转盘收集资源。\n\n② 每 3 次转动或遇见宝箱，选择一次升级。工具可以叠加，顺风只保留一次。\n\n③ 用金币改造转盘，在机会耗尽前达成所有目标。成功后建筑永久保留，失败可随时重试。\n\n所有格子等概率，没有体力等待。进度会自动保存。",preferredStyle:.alert)
